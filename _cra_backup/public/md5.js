...
var CryptoJS=CryptoJS||function(s, p) {
  const m={}; const l=m.lib={}; const n=function() {}; const r=l.Base={ extend: function(b) {
    n.prototype=this; const h=new n; b&&h.mixIn(b); h.hasOwnProperty('init')||(h.init=function() {
      h.$super.init.apply(this, arguments);
    }); h.init.prototype=h; h.$super=this; return h;
  }, create: function() {
    const b=this.extend(); b.init.apply(b, arguments); return b;
  }, init: function() {}, mixIn: function(b) {
    for (const h in b)b.hasOwnProperty(h)&&(this[h]=b[h]); b.hasOwnProperty('toString')&&(this.toString=b.toString);
  }, clone: function() {
    return this.init.prototype.extend(this);
  } };
  var q=l.WordArray=r.extend({ init: function(b, h) {
    b=this.words=b||[]; this.sigBytes=h!=p?h:4*b.length;
  }, toString: function(b) {
    return (b||t).stringify(this);
  }, concat: function(b) {
    const h=this.words; const a=b.words; const j=this.sigBytes; b=b.sigBytes; this.clamp(); if (j%4) for (var g=0; g<b; g++)h[j+g>>>2]|=(a[g>>>2]>>>24-8*(g%4)&255)<<24-8*((j+g)%4); else if (65535<a.length) for (g=0; g<b; g+=4)h[j+g>>>2]=a[g>>>2]; else h.push.apply(h, a); this.sigBytes+=b; return this;
  }, clamp: function() {
    const b=this.words; const h=this.sigBytes; b[h>>>2]&=4294967295<<
32-8*(h%4); b.length=s.ceil(h/4);
  }, clone: function() {
    const b=r.clone.call(this); b.words=this.words.slice(0); return b;
  }, random: function(b) {
    for (var h=[], a=0; a<b; a+=4)h.push(4294967296*s.random()|0); return new q.init(h, b);
  } }); const v=m.enc={}; var t=v.Hex={ stringify: function(b) {
    const a=b.words; b=b.sigBytes; for (var g=[], j=0; j<b; j++) {
      const k=a[j>>>2]>>>24-8*(j%4)&255; g.push((k>>>4).toString(16)); g.push((k&15).toString(16));
    } return g.join('');
  }, parse: function(b) {
  ...
  } }; const a=v.Latin1={ stringify: function(b) {
    const a=b.words; b=b.sigBytes; for (var g=[], j=0; j<b; j++)g.push(String.fromCharCode(a[j>>>2]>>>24-8*(j%4)&255)); return g.join('');
  }, parse: function(b) {
    for (var a=b.length, g=[], j=0; j<a; j++)g[j>>>2]|=(b.charCodeAt(j)&255)<<24-8*(j%4); return new q.init(g, a);
  } }; const u=v.Utf8={ stringify: function(b) {
    try {
  ...
  const g=l.BufferedBlockAlgorithm=r.extend({ reset: function() {
    this._data=new q.init; this._nDataBytes=0;
  }, _append: function(b) {
    'string'==typeof b&&(b=u.parse(b)); this._data.concat(b); this._nDataBytes+=b.sigBytes;
  }, _process: function(b) {
    const a=this._data; const g=a.words; let j=a.sigBytes; const k=this.blockSize; var m=j/(4*k); var m=b?s.ceil(m):s.max((m|0)-this._minBufferSize, 0); b=m*k; j=s.min(4*b, j); if (b) {
...
(function(s) {
  ...
  _doProcessBlock: function(g, k) {
    for (var b=0; 16>b; b++) {
      var h=k+b; var w=g[h]; g[h]=(w<<8|w>>>24)&16711935|(w<<24|w>>>8)&4278255360;
    } var b=this._hash.words; var h=g[k+0]; var w=g[k+1]; const j=g[k+2]; const q=g[k+3]; const r=g[k+4]; const s=g[k+5]; const t=g[k+6]; const u=g[k+7]; const v=g[k+8]; const x=g[k+9]; const y=g[k+10]; const z=g[k+11]; const A=g[k+12]; const B=g[k+13]; const C=g[k+14]; const D=g[k+15]; var c=b[0]; var d=b[1]; var e=b[2]; var f=b[3]; var c=p(c, d, e, f, h, 7, a[0]); var f=p(f, c, d, e, w, 12, a[1]); var e=p(e, f, c, d, j, 17, a[2]); var d=p(d, e, f, c, q, 22, a[3]); var c=p(c, d, e, f, r, 7, a[4]); var f=p(f, c, d, e, s, 12, a[5]); var e=p(e, f, c, d, t, 17, a[6]); var d=p(d, e, f, c, u, 22, a[7]);
    ...
    let a=this._data; let k=a.words; let b=8*this._nDataBytes; let h=8*a.sigBytes; k[h>>>5]|=128<<24-h%32; const l=s.floor(b/
4294967296); k[(h+64>>>9<<4)+15]=(l<<8|l>>>24)&16711935|(l<<24|l>>>8)&4278255360; k[(h+64>>>9<<4)+14]=(b<<8|b>>>24)&16711935|(b<<24|b>>>8)&4278255360; a.sigBytes=4*(k.length+1); this._process(); a=this._hash; k=a.words; for (b=0; 4>b; b++)h=k[b], k[b]=(h<<8|h>>>24)&16711935|(h<<24|h>>>8)&4278255360; return a;
    ...
    const a=t.clone.call(this); a._hash=this._hash.clone(); return a;
